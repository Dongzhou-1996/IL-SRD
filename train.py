...
class train_IL_SRD(object):
    def __init__(self, hidden_dim, seq_len, input_dim, output_dim, nhead, num_encoder_layers, num_decoder_layers,
        ...
        self.policy = IL_SRD(hidden_dim, seq_len, input_dim, output_dim, nhead, num_encoder_layers,
    ...
    def eval_new(self, max_timesteps, seed):
        self.policy = IL_SRD(self.hidden_dim, self.seq_len, self.input_dim, self.output_dim, self.nhead,
        ...
        il_input, _ = normalization(self.norm_method, obs.detach().clone(), self.norm_dict['state'])
        il_input = torch.tensor(il_input.detach().clone().requires_grad_(True), device=self.device).to(
            torch.float32)
        ...
        with torch.inference_mode():
            for t in range(max_timesteps):
                all_actions, _, _ = self.policy(il_input, actions=None)
